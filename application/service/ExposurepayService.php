...
class ExposurepayService
{
    ...
    public static function ExposureList($params = [])
    {
        ...
        $data = Db::name('Exposurepay')->field($field)->where($where)->order($order_by)->select();
    ...
    public static function ExposureDelete($params = [])
    {
        ...
        // 删除操作
        if(Db::name('Exposurepay')->where(['id'=>$params['ids']])->update(['is_delete_time'=>time(), 'upd_time'=>time()]))
        {
