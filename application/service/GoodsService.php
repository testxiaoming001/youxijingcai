...
class GoodsService
{
    ...
    {
        $where['is_enable'] = 1;
        $field              = 'id,pid,level,icon,name,vice_name,describe,bg_color,big_images,sort,is_home_recommended,seo_title,seo_keywords,seo_desc';
     ...
     * @blog    http://gong.gg/
     */
    public static function HomeList($params = [])
    {
        // 商品数量
        $goods_count = 16;


        $data = self::GoodsList(['where' => ['is_home_recommended' => 1, 'is_shelves' => 1], 'm' => 0, 'n' => $goods_count, 'field' => '*']);
...
//        echo json_encode($data);die();
        ...
        if (!empty($data)) {
            ...
            foreach ($data as $k => &$v) {
                ...
                if (isset($v['place_origin'])) {
                    $v['place_origin_name'] = empty($v['place_origin']) ? '不支持同台' : RegionService::RegionName($v['place_origin']);
                }
                ...
                //商品的担保方
                $v['assureStyles'] = self::getGoodsAssureStyles($v['id']);
                //商品分类
                $cates                    = self::getGoodsCateGory($v['id']);
                $data[$k]['cates_router'] = implode(' | ', $cates);
            }
        }
     ...
     *查询商品所属分类
     */
    public static function getGoodsCateGory($goodsId)
    {

        return Db::name('GoodsCategoryJoin')->alias('a')
                 ->field('b.id,b.pid,b.name,b.level')
                 ->join('GoodsCategory b', 'a.category_id=b.id', 'left')
     ...
     *查询商品所属分类id
     */
    public static function getGoodsCateGoryIds($goodsId)
    {
     ...
     */
    public static function getGoodsAssureStyles($goodsId)
    {
        $types              = Db::name('GoodsAssureStyle')->where(['goods_id' => $goodsId])->select();
        $assureCommonStyles = lang('common_assure_style_list');
        if ($types) {
            $types = array_map(function ($param) use ($assureCommonStyles) {
                $param['assure_style_name'] = $assureCommonStyles[$param['assure_style_id']]['name'];
                return $param;
            }, $types);
        }
        return $types;
    ...
    public static function getSeriesNumber()
    {
        return '20400';
    }
     ...
     * @blog    http://gong.gg/
     */
    public static function GoodsList($params = [], $debug = false)
    {
        $where    = empty($params['where']) ? [] : $params['where'];
        ...
        if ($debug) {
            $sql  = Db::name('Goods')->field($field)->where($where)->order($order_by)->limit($m, $n)->buildSql();
            $data = Db::query($sql);
        }
     ...
     * @param array $cateIds
     */
    protected static function getGoodsCatePidsByCateIds($cateIds = [])
    {
        return Db::name('GoodsCategory')->wherein('id', $cateIds)
                 ->column('pid');
    }
     ...
     * @param $uid
     */
    protected static function isGoodsCanEngoreCheck($uid)
    {

        $reliabilityLevel      = 8;
        $isGoodsCanEngoreCheck = true;
        $uid && Db::name('User')->where([
            'id' => $uid
        ])->value('user_reliability') < $reliabilityLevel && $isGoodsCanEngoreCheck = false;

        return $isGoodsCanEngoreCheck;
...
//         请求参数
        $p   = [
            ...
            [
                'checked_type' => 'length',
                'key_name'     => 'goods_type_info',
                'checked_data' => '60',
                //                'is_checked'        => 1,
                'error_msg'    => '供需说明格式 最多60个字符',
            ],
            [
                'checked_type' => 'length',
                ...
                'checked_data' => '160',
                'is_checked'   => 1,
                'error_msg'    => '买/卖家telegram账号 最多160个字符',
            ],
            [
                'checked_type' => 'empty',
                'key_name'     => 'user_telegram',
                'error_msg'    => '请输入买/卖家telegram账号',
            ],
            ...
            //            [
            //                'checked_type' => 'in',
            //                'key_name' => 'assure_style',
            //                'checked_data' => array_merge([-1], array_column(lang('common_assure_style_list'), 'value')),
            //                'is_checked' => 2,
            //                'error_msg' => '请选择担保方式',
            //            ],
            ...
            [
                'checked_type' => 'in',
                'key_name'     => 'goods_type',
                'checked_data' => array_merge([-1], array_column(lang('common_goods_type_list'), 'value')),
                'is_checked'   => 2,
                'error_msg'    => '请选择供需类型',
            ],
        ...
        $goodsCateIds = explode(',', $params['category_id']);
        $pids         = array_unique(self::getGoodsCatePidsByCateIds($goodsCateIds));
        $pids         = array_unique(self::getGoodsCatePidsByCateIds($pids));
        if (count($pids) > 1) {
            return DataReturn('只能选择同一一级分类下的多个三级分类', -1);
        }
...
//        }
        ...
        $userTelegram = str_replace('@', '', $params['user_telegram']);

        $uid = isset($params['user']) ? intval($params['user']['id']) : 0;
        ...
        $data = [
            ...
            'place_area'             => empty($params['place_area']) ? '' : $params['place_area'],
            'simple_desc'            => $params['simple_desc'],
            'goods_type_info'            => $params['goods_type_info'],
            ...
            //            'assure_style'                 => isset($params['assure_style']) ? $params['assure_style'] : -1,
            'goods_type'             => isset($params['goods_type']) ? $params['goods_type'] : -1,
            'deposit'                => isset($params['deposit']) ? $params['deposit'] : '0.00',
            'goods_price'            => isset($params['goods_price']) ? $params['goods_price'] : '0.00',
            'goods_rate'             => isset($params['goods_rate']) ? $params['goods_rate'] : '0.00',
            'goods_reliability'      => isset($params['goods_reliability']) ? $params['goods_reliability'] : '0',
            'uid'                    => $uid,
            'limit_moneys'      => isset($params['limit_moneys']) ? $params['limit_moneys'] : '',
            'day_order_count'      => isset($params['day_order_count']) ? $params['day_order_count'] : 0,
            'is_one_day'      => isset($params['is_one_day']) ? $params['is_one_day'] : 0,
        ...
        if (empty($params['id'])) {
            $data['add_time']     = time();
            $data['upd_time']     = time();
            $data['check_status'] = self::isGoodsCanEngoreCheck($uid) ? 1 : 0;
            $goods_id             = Db::name('Goods')->insertGetId($data);
        } else {
            $goods                = Db::name('Goods')->find($params['id']);
            $goods['uid'] && $data['uid'] = $goods['uid'];
            $data['upd_time']     = time();
            $data['check_status'] = isset($params['check_status']) ? intval($params['check_status']) : 0;
        ...
        if (isset($goods_id) && $goods_id > 0) {
            ...
            //担保方式
            $ret = self::GoodsSaveAssureStyle($params, $goods_id);
            if ($ret['code'] != 0) {
                // 回滚事务
                Db::rollback();
                return $ret;
...
//            }
            ...
            if(!$uid){
                //后台添加
                if (empty($params['id']) && $data['check_status'] ) {
                    $is_bot_send = true;
                }else{
                    //审核
                    if(!$uid && $data['check_status']  ){

                        $goods = Db::name('Goods')->where(['id' => $goods_id])->find();
                        if(!$goods['check_success_time']) {
                            Db::name('Goods')->where(['id' => $goods_id])->update(['check_success_time'=>time()]);
                        }
                    }
                }
            ...
            if($is_bot_send) {
                ...
                $res = $TelegramBotService->publishGoodsMessage($goods_id);
                if (!$res['ok']) {
                    Db::rollback();
                    return DataReturn($res['description'], -1);
                }
            }
     ...
     * 担保人
     */
    public static function GoodsSaveAssureStyle($params, $goods_id)
    {
        //删除之前的
        Db::name('GoodsAssureStyle')->where(['goods_id' => $goods_id])->delete();
        if (isset($params['assure_style']) && $params['assure_style']) {
            //支持担保
            //添加现在的
            $array = explode(',', $params['assure_style']);
            if (in_array(0, $array) && count($array) > 0) {
                return DataReturn('您已选择不支持担保不可与支持担保方同时提交', -1);
            }
            $install = [];
            ...
            if ($install) {
                $res = Db::name('GoodsAssureStyle')->insertAll($install);
                if (!$res) {
...
//        }
    ...
    public static function GoodsCategoryNodeSon($params = [])
    {
        ...
        // 获取数据
        $field = 'id,pid,rate,limit_moneys,is_one_day,day_order_count,icon,name,sort,is_enable,bg_color,big_images,vice_name,describe,is_home_recommended,seo_title,seo_keywords,seo_desc';
        $data  = Db::name('GoodsCategory')->field($field)->where(['pid' => $id])->order('sort asc')->select();
        if (!empty($data)) {
            foreach ($data as &$v) {
                $ids                       = self::getCategoryTreeIds($v['id']);
                $v['day_new_number']       = self::getDayNewNumber($ids);
                $v['yesterday_new_number'] = self::getYesterDayNewNumber($ids);
     ...
     * 获取当日新增数量
     */
    public static function getDayNewNumber($ids)
    {
        return Db::name('GoodsCategoryJoin')
                 ->join('s_goods', 's_goods.id = s_goods_category_join.goods_id')
                 ->group('s_goods.id')
                 ->whereTime('s_goods.add_time', 'today')->where('s_goods_category_join.category_id', 'in', $ids)->count();
    }

    /**
     * 获取昨日新增数量
     */
    public static function getYesterDayNewNumber($ids)
    {
        return Db::name('GoodsCategoryJoin')
                 ->join('s_goods', 's_goods.id = s_goods_category_join.goods_id')
                 ->group('s_goods.id')
                 ->whereTime('s_goods.add_time', 'yesterday')->where('s_goods_category_join.category_id', 'in', $ids)->count();
    }
    ...
    {
        $data    = Db::name('GoodsCategory')->field('id,pid')->where(['is_enable' => '1'])->select();
        $ids     = [];
        $array   = self::getTree($data, $id, $ids);
        $array[] = $id;
        return $array;
    }

    public static function getTree($data, $id, &$ids)
    {
        foreach ($data as $k => $v) {
            if ($v['pid'] == $id) {
                self::getTree($data, $v['id'], $ids);
                $ids[] = $v['id'];
            }
        }
        return $ids;
    ...
    public static function getGoodsCategoryLevelByPid($pid, &$level = 1)
    {
        $father = Db::name('GoodsCategory')->where('id', '=', $pid)->find();
        if ($father['pid'] != 0) {
            $level++;
            self::getGoodsCategoryLevelByPid($father['pid'], $level);
        }
    ...
    public static function GoodsCategorySave($params = [])
    {
        ...
        $level = self::getGoodsCategoryLevelByPid($params['pid']);
        // 数据
        $data = [
            'name'                => $params['name'],
            'pid'                 => $pid,
            'level'               => $level,
            'is_one_day'          => isset($params['is_one_day']) ? $params['is_one_day'] : 0,
            'limit_moneys'        => trim($params['limit_moneys']),
            'day_order_count'     => isset($params['day_order_count']) ? $params['day_order_count'] : 0,
    ...
    {
        return Db::name('Goods')->where(['id' => $goodsId])->find();
    }
     ...
     * @param $childId
     */
    public static  function getTopParentBychildsId($childPid)
    {
        $father = Db::name('GoodsCategory')->where(['id'=>$childPid])->find();
        if($father['pid']  != 0)
        {

            return  self::getTopParentBychildsId($father['pid']);
        }
        return $father;
    }
}
