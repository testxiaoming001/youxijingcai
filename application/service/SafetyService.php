...
class SafetyService
{
     ...
     */
    public static function PayPwdUpdate($params = [])
    {
        // 数据验证
        $p = [
            [
                'checked_type'      => 'length',
                'checked_data'      => '6,18',
                'key_name'          => 'new_pay_pwd',
                'error_msg'         => '新密码格式 6~18 个字符之间',
            ],
            [
                'checked_type'      => 'length',
                'checked_data'      => '6,18',
                'key_name'          => 'confirm_new_pay_pwd',
                'error_msg'         => '确认密码格式 6~18 个字符之间，与新密码一致',
            ],
            [
                'checked_type'      => 'empty',
        ...
        // 确认密码是否相等
        if($params['new_pay_pwd'] != $params['confirm_new_pay_pwd'])
        {
            return DataReturn('两次密码不一致', -5);
        }

        // 密码修改
        $accounts = empty($user['mobile']) ? (empty($user['email']) ? $user['username'] : $user['email']) : $user['mobile'];
        $ret = self::UserPayPwdUpdate($accounts, $user['id'], $params['new_pay_pwd']);
        if($ret['code'] != 0)
        {
            return DataReturn('操作成功', 0);
        }
     ...
     */
    public static function UserPayPwdUpdate($accounts, $user_id, $pwd)
    {
        $salt = GetNumberCode(6);
        $data = array(
            'pay_pwd'       =>  LoginPwdEncryption(trim($pwd), $salt),
            'salt'      =>  $salt,
            'upd_time'  =>  time(),
        );
        if(Db::name('User')->where(['id'=>$user_id])->update($data) !== false)
