...
class Goods extends Common
{
    ...
    public function Index()
    {
        ...
        foreach ($this->form_where as $k => $v) {
            if (isset($v[0]) && $v[0] == 'id') {
                $this->form_where[$k][2] = substr($v[2], 5);
            }
        }
        // 总数
    ...
    public function getTopParents()
    {
        ...
        $childIds = input('cate_ids',[]);
        if(empty($childIds))
        {
            $father = [];
        }else
        {
            $self = Db::name('GoodsCategory')->where(['id'=>$childIds[0]])->find();
            $father =  GoodsService::getTopParentBychildsId($self['pid']);
        }
        return $father;
    }
}
