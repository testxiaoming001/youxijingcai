		...
		success:function(result)
		{
			if(result.code == 0 && result.data.length > 0)
			{
				...
				for(var i in result.data)
				{
					...
					if(result.data[i].hasOwnProperty("day_new_number")) {
                        html += '<span style="margin-left: 20%"  >今日新增：' + result.data[i]['day_new_number'];
                        html += '</span>';
                    }
                    if(result.data[i].hasOwnProperty("yesterday_new_number")) {
                        html += '<span style="margin-left: 20%">昨日新增：' + result.data[i]['yesterday_new_number'];
                        html += '</span>';
                    }

                    if(result.data[i].hasOwnProperty("service_telegram")) {
                        html += '<span style="margin-left: 20%">telegram账号：' + result.data[i]['service_telegram'];
                        html += '</span>';
                    }
					// 数据 end
...
$(function()
{
    ...
    $('.search').on('click',function () {
		$('.searchlist').css('display',($('.searchlist').css('display') == 'none')?'block':'none')
	})


	$('.searchlist').find('li').click(function(){
		console.log($(this).attr('search_type'));
		$("input[name='goods_type']").val($(this).attr('search_type'));
		$('.search').find('cite').text($(this).text());
	})
